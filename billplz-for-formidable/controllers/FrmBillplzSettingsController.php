<?php
class FrmBillplzSettingsController{
    public static function load_hooks(){
        add_action('frm_add_settings_section', 'FrmBillplzSettingsController::add_settings_section'); // global settings
        add_action('frm_after_duplicate_form', 'FrmBillplzSettingsController::duplicate', 15, 2);

        // < 2.0 fallback
		add_action( 'frm_entry_form', 'FrmBillplzPaymentsController::hidden_payment_fields' );

        // 2.0 hooks
		add_action( 'frm_registered_form_actions', 'FrmBillplzSettingsController::register_actions' );
		add_action( 'frm_add_form_option_section', 'FrmBillplzSettingsController::actions_js' );
		add_action( 'frm_before_list_actions', 'FrmBillplzSettingsController::migrate_to_2' );

		add_filter( 'frm_action_triggers', 'FrmBillplzSettingsController::add_payment_trigger' );
		add_filter( 'frm_email_action_options', 'FrmBillplzSettingsController::add_trigger_to_action' );
		add_filter( 'frm_twilio_action_options', 'FrmBillplzSettingsController::add_trigger_to_action' );
		add_filter( 'frm_mailchimp_action_options', 'FrmBillplzSettingsController::add_trigger_to_action' );
		add_filter( 'frm_register_action_options', 'FrmBillplzSettingsController::add_payment_trigger_to_register_user_action' );
    }

    public static function add_settings_section($sections){
        $sections['billplz'] = array('class' => 'FrmBillplzSettingsController', 'function' => 'route');
        return $sections;
    }
    ...
    public static function duplicate($id, $values) {
        ...
        if ( !isset($values['options']['billplz_item_name']) || empty($values['options']['billplz_item_name']) ) {
            // don't continue if there aren't billplz settings to switch
            return;
        }
        
        global $frm_duplicate_ids;
        
        if ( is_numeric($new_opts['billplz_amount_field']) && isset($frm_duplicate_ids[$new_opts['billplz_amount_field']]) ) {
            $new_opts['billplz_amount_field'] = $frm_duplicate_ids[$new_opts['billplz_amount_field']];
        }
        
        $new_opts['billplz_item_name'] = FrmProFieldsHelper::switch_field_ids($new_opts['billplz_item_name']);
        
        // switch conditional logic
        if ( is_array($new_opts['billplz_list']) && isset($new_opts['billplz_list']['hide_field']) ) {
            foreach ( (array) $new_opts['billplz_list']['hide_field'] as $ck => $cv ) {
                if ( is_numeric($cv) && isset($frm_duplicate_ids[$cv]) ) {
                    $new_opts['billplz_list']['hide_field'][$ck] = $frm_duplicate_ids[$cv];
                }
    ...
    public static function register_actions($actions) {
        $actions['billplz'] = 'FrmBillplzAction';
        return $actions;
    }
	...
	public static function add_payment_trigger( $triggers ) {
		$triggers['billplz'] = __( 'Billplz payment', 'frmbz' );
